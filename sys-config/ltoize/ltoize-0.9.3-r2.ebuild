...
pkg_postinst()
{
	...
	if [[ -z "${LD_VER}" ]]; then
		ewarn "Warning: active linker is not ld.gold, it is highly recommended to set ld.gold as your default system linker."
		ewarn "You can do this by using USE=default-gold on sys-devel/binutils OR by setting binutils-config --linker ld.gold"
	fi
